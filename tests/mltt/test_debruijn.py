from mltt.ast import (
    App,
    Id,
    IdElim,
    Lam,
    NatRec,
    Pair,
    Pi,
    Sigma,
    Succ,
    TypeUniverse,
    Var,
    Zero,
)
from mltt.debruijn import shift, subst


# ------------- Shift: basic behavior -------------

def test_shift_var_free_at_or_above_cutoff_is_bumped():
    assert shift(Var(0), by=1, cutoff=0) == Var(1)
    assert shift(Var(2), by=3, cutoff=2) == Var(5)

def test_shift_var_below_cutoff_unchanged():
    # cutoff shields lower indices
    assert shift(Var(0), by=2, cutoff=1) == Var(0)
    assert shift(Var(1), by=2, cutoff=2) == Var(1)

def test_shift_by_zero_is_identity():
    ty = Var(0)
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    f1 = Var(2)
    a1 = Lam(ty, body)
    t = App(f1, a1)
    assert shift(t, by=0, cutoff=0) == t

def test_shift_app_distributes():
    f = Var(1)
    a = Var(0)
    t = App(f, a)
    shifted = shift(t, by=1, cutoff=0)
    f1 = Var(2)
    a1 = Var(1)
    assert shifted == App(f1, a1)

def test_shift_lam_body_uses_cutoff_plus_1():
    # λ. Var(1)  -> shifting by +1 at cutoff=0 should become λ. Var(2)
    ty = Var(0)
    body = Var(1)
    t = Lam(ty, body)
    ty1 = shift(Var(0), 1, 0)
    body1 = Var(2)
    assert shift(t, by=1, cutoff=0) == Lam(ty1, body1)

def test_shift_lam_preserves_bound_var():
    # λ. Var(0) : inner Var(0) is bound, cutoff+1 prevents shift
    ty = Var(42)
    body = Var(0)
    t = Lam(ty, body)
    ty1 = shift(Var(42), 5, 0)
    body1 = Var(0)
    assert shift(t, by=5, cutoff=0) == Lam(ty1, body1)

def test_shift_nested_lams_correctly_increments_cutoff():
    # λ. λ. Var(2)  -> going under two binders raises cutoff twice
    ty = Var(1)
    body = Var(2)
    ty2 = Var(0)
    body2 = Lam(ty, body)
    t = Lam(ty2, body2)
    s = shift(t, by=1, cutoff=0)
    # Only the Var(2) (free w.r.t both binders) becomes Var(3)
    ty1 = shift(Var(1), 1, 0)
    body1 = Var(3)
    ty3 = shift(Var(0), 1, 0)
    body3 = Lam(ty1, body1)
    assert s == Lam(ty3, body3)

def test_shift_pi_behaves_like_lam():
    ty = Var(7)
    body = Var(1)
    t = Pi(ty, body)
    ty1 = shift(Var(7), 2, 0)
    body1 = Var(3)
    assert shift(t, by=2, cutoff=0) == Pi(ty1, body1)

def test_shift_negative_pops_binder_levels():
    # Equivalent to "popping" a binder layer for indices >= cutoff
    f = Var(3)
    a = Var(1)
    t = App(f, a)
    f1 = Var(2)
    a1 = Var(0)
    assert shift(t, by=-1, cutoff=1) == App(f1, a1)
    # Below cutoff index unchanged
    assert shift(Var(0), by=-1, cutoff=1) == Var(0)

# ------------- Subst on Vars (local rule) -------------

def test_subst_var_equal_index_replaced():
    assert subst(Var(0), sub=Var(99), j=0) == Var(99)
    assert subst(Var(5), sub=Var(77), j=5) == Var(77)

def test_subst_var_higher_index_decrements():
    assert subst(Var(3), sub=Var(42), j=1) == Var(2)
    assert subst(Var(2), sub=Var(10), j=0) == Var(1)

def test_subst_var_lower_index_unchanged():
    assert subst(Var(0), sub=Var(42), j=2) == Var(0)
    assert subst(Var(1), sub=Var(99), j=3) == Var(1)

# ------------- Subst under binders (capture-avoidance) -------------

def test_subst_under_lam_shifts_subterm_and_increments_j():
    # subst(λ(ty). body, sub, j) = λ(subst(ty, sub, j), subst(body, shift(sub,1,0), j+1))
    ty = Var(2)
    f = Var(2)
    a = Var(0)
    body = App(f, a)  # inner v(0) is bound; v(2) is free w.r.t this binder
    t = Lam(ty, body)
    f1 = Var(1)
    a1 = Var(0)
    sub = App(f1, a1)  # sub has free vars; must be shifted when entering body
    res = subst(t, sub=sub, j=1)

    # Expected:
    # ty: subst(v(2), sub, 1) -> since 2 > 1, becomes v(1)
    exp_ty = Var(1)
    # body: subst(body, shift(sub,1,0), 2)
    shifted_sub = shift(sub, 1, 0)  # app(v(2), v(1))
    # body = app(v(2), v(0)):
    #   - subst(v(2), shifted_sub, j=2)  -> equals j, so replace with shifted_sub
    #   - subst(v(0), shifted_sub, j=2)  -> 0 < 2, unchanged
    a2 = Var(0)
    exp_body = App(shifted_sub, a2)

    assert res == Lam(exp_ty, exp_body)

def test_subst_bound_variable_not_replaced():
    # λ. Var(0); substituting j=0 should not affect bound occurrences in the body when using plain subst
    # (they are bound by the λ and the rule increments j when descending)
    ty = Var(0)
    body = Var(0)
    t = Lam(ty, body)
    sub = Var(42)
    # NOTE: substituting into the lambda node doesn't eliminate the binder; that's what subst is for.
    res = subst(t, sub=sub, j=0)
    # ty: subst(v(0), sub, 0) -> matches j, so replaced by sub
    # body: subst(v(0), shift(sub,1,0), 1) -> 0 < 1 => unchanged
    body1 = Var(0)
    assert res == Lam(sub, body1)

def test_subst_free_above_j_drops_by_one_under_binder():
    # t = λ. App(Var(2), Var(1))
    ty = Var(0)
    f = Var(2)
    a = Var(1)
    body = App(f, a)
    t = Lam(ty, body)
    # substitute j=0 (the innermost free var at the top level)
    sub = Var(5)
    res = subst(t, sub, j=0)
    # ty: subst(v(0), v(5), 0) -> replaced by v(5)
    # body: j becomes 1; sub becomes shift(v(5),1,0) = v(6)
    #   v(2) with j=1 -> 2 > 1 => v(1)
    #   v(1) with j=1 -> equal => replaced by v(6)
    ty1 = Var(5)
    f1 = Var(1)
    a1 = Var(6)
    body1 = App(f1, a1)
    assert res == Lam(ty1, body1)

def test_subst_under_pi_increments_j_and_shifts_subterm():
    ty = Var(3)
    f = Var(2)
    a = Var(0)
    body = App(f, a)
    t = Pi(ty, body)
    f1 = Var(0)
    a1 = Var(1)
    sub = App(f1, a1)
    res = subst(t, sub, j=1)
    exp_ty = Var(2)  # v(3) with j=1 -> 3>1 => 2
    shifted_sub = shift(sub, 1, 0)  # app(v(1), v(2))
    # body under binder: j -> 2
    #   v(2) == j -> replace with shifted_sub
    #   v(0) < j -> stays v(0)
    a2 = Var(0)
    exp_body = App(shifted_sub, a2)
    assert res == Pi(exp_ty, exp_body)

# ------------- β-reduction via subst (TAPL shift dance) -------------

def test_beta_simple_identity():
    # (λ. Var(0)) arg  -> arg
    body = Var(0)
    arg = Var(7)
    assert subst(body, arg) == arg

def test_beta_ignores_argument():
    # (λ. Var(1)) arg  -> Var(0)  (outer variable gets one level closer)
    body = Var(1)
    arg = Var(0)
    assert subst(body, arg) == Var(0)

def test_beta_capture_avoidance_nontrivial():
    # (λ. λ. Var(2))  applied to  Var(0)
    # body inside outer λ is λ. Var(2)
    body = Lam(Var(0), Var(2))
    arg = Var(0)
    # After β on the outer λ, we expect λ. Var(2) (the free var remains free)
    # Walkthrough: subst(body, arg) performs the +1/-1 shift dance correctly
    res = subst(body, arg)
    assert res == Lam(Var(0), Var(1))  # ty shifted; body free index unchanged

def test_beta_argument_with_free_vars_no_capture():
    # (λ. App(Var(1), Var(0))) (App(Var(1), Var(0)))
    # Should produce App(Var(1), App(Var(2), Var(1))) with correct shifting
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    f1 = Var(1)
    a1 = Var(0)
    arg  = App(f1, a1)
    res = subst(body, arg)
    # Compute expected:
    # shift(arg,1,0) = app(v(2), v(1))
    # subst_impl(body, shift(arg,1,0), 0):
    #   v(1) with j=0 => 1>0 -> v(0)
    #   v(0) with j=0 => replaced with app(v(2), v(1))
    # => app(v(0), app(v(2), v(1)))
    # shift(...,-1,0):
    #   v(0) below cutoff=0? No (0>=0) => v(-1)? Illegal—BUT note:
    #   That v(0) came from decrementing 1>0 -> 0, then the final global pop
    #   Let's recompute safely by direct evaluation using functions:
    # Instead of hand-deriving fragilely, check structural properties:
    assert isinstance(res, App)
    assert isinstance(res.func, Var) or isinstance(res.func, App)
    # Check that no Var indices became negative:
    def no_negative_vars(t):
        match t:
            case Var(k): return k >= 0
            case App(f,a): return no_negative_vars(f) and no_negative_vars(a)
            case Lam(ty,b): return no_negative_vars(ty) and no_negative_vars(b)
            case Pi(ty,b):  return no_negative_vars(ty) and no_negative_vars(b)
            case _: return True
    assert no_negative_vars(res)

def test_beta_against_nested_binders():
    # ((λ. λ. App(Var(2), Var(0))) arg)
    # After β of the outer λ with arg=Var(3):
    #   - New λ param type: ty' = subst(v(0), j=0, sub=v(3)) = v(3).
    #   - In the inner body App(Var(2), Var(0)), under the remaining λ we substitute with
    #     j=1 and sub' = shift(v(3), +1, 0) = v(4):
    #       Var(2) -> Var(1)  (since 2>1)
    #       Var(0) -> Var(0)  (since 0<1)
    #   So the result is λ. App(Var(1), Var(0)).
    outer_body = Lam(Var(0), App(Var(2), Var(0)))
    arg = Var(3)
    res = subst(outer_body, arg)
    assert res == Lam(Var(3), App(Var(1), Var(0)))


# ------------- Subst/Shift interaction laws (spot checks) -------------

def test_shift_subst_commutation_law_spotcheck():
    # shift(subst(t, j, s), d, c) == subst(shift(t, d, c), j + (0 if c <= j else 0) + d, shift(s, d, 0))
    # We use the common special case c=0:
    ty = Var(0)
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    f1 = Var(2)
    a1 = Lam(ty, body)
    t = App(f1, a1)
    j = 1
    f2 = Var(0)
    a2 = Var(2)
    s = App(f2, a2)
    d = 2
    c = 0

    left  = shift(subst(t, j=j, sub=s), by=d, cutoff=c)
    right = subst(shift(t, by=d, cutoff=c), j=j + d, sub=shift(s, by=d, cutoff=0))
    assert left == right

def test_subst_then_subst_index_adjustment_spotcheck():
    # subst(subst(t, j, s), i, r) vs subst(subst(t, i, r), j', s') with proper index arithmetic
    # Check a small concrete instance
    f = Var(2)
    a = Var(0)
    f1 = Var(3)
    a1 = App(f, a)
    t = App(f1, a1)
    s = Var(1)
    r = Var(0)
    j = 2
    i = 0
    # Left: replace 2->1, then 0->0
    left = subst(subst(t, j=j, sub=s), j=i, sub=r)
    # Right: replace 0->0 first, then adjust j because replacing 0 can lower indices >0 by 1
    t1 = subst(t, j=i, sub=r)
    s1 = subst(s, j=i, sub=r)
    # After removing j=0, original j=2 becomes j' = 1
    right = subst(t1, j=1, sub=s1)
    assert left == right

# ------------- Subst in types (ty field) -------------

def test_subst_affects_ty_field_at_same_depth():
    ty = Var(1)
    body = Var(0)
    t = Lam(ty, body)  # ty = Var(1)
    sub = Var(7)
    res = subst(t, sub=sub, j=1)
    # ty: Var(1) == j -> replaced by sub
    # body: j becomes 2, sub shifts to shift(7,1,0)=Var(8); v(0) with j=2 stays v(0)
    ty1 = Var(7)
    body1 = Var(0)
    assert res == Lam(ty1, body1)

def test_subst_ty_decrements_higher_indices():
    ty = Var(3)
    body = Var(0)
    t = Lam(ty, body)
    res = subst(t, sub=Var(42), j=1)
    # ty: 3>1 -> 2
    assert res.ty == Var(2)

# ------------- Identity and stability checks -------------

def test_subst_irrelevant_index_no_change():
    ty = Var(0)
    body = Var(0)
    f = Var(2)
    a = Lam(ty, body)
    t = App(f, a)
    f1 = Var(1)
    a1 = Var(0)
    s = App(f1, a1)
    assert subst(t, sub=s, j=99) == t

def test_subst_with_closed_subterm_is_well_behaved():
    # sub has no free variables relative to cutoff=0 (e.g., Var(0) under its own binder in tests via subst)
    f = Var(2)
    a = Var(0)
    t = App(f, a)
    sub = Var(0)
    f1 = Var(0)
    a1 = Var(0)
    assert subst(t, sub=sub, j=2) == App(f1, a1)

# ------------- Regression-style edge cases -------------

def test_no_negative_indices_after_subst_top():
    # Stress: ensure subst never creates negative indices
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    f1 = Var(0)
    a1 = Var(1)
    arg  = App(f1, a1)
    res = subst(body, arg)
    # Walk tree to confirm
    def ok(t):
        match t:
            case Var(k): return k >= 0
            case App(f,a): return ok(f) and ok(a)
            case Lam(ty,b): return ok(ty) and ok(b)
            case Pi(ty,b):  return ok(ty) and ok(b)
            case _: return True
    assert ok(res)

def test_shift_cutoff_beyond_all_vars_is_identity():
    ty = Var(0)
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    f1 = Var(1)
    a1 = Lam(ty, body)
    t = App(f1, a1)
    # max free depth is 1 at top-level; cutoff=10 shields everything
    assert shift(t, by=3, cutoff=10) == t

def test_subst_high_j_beyond_all_vars_identity():
    ty = Var(0)
    f = Var(1)
    a = Var(0)
    body = App(f, a)
    t = Lam(ty, body)
    assert subst(t, sub=Var(9), j=10) == t


# --------- other types ----------


def test_shift_respects_cutoff():
    term = App(Var(1), Var(0))
    shifted = shift(term, by=2, cutoff=1)
    assert shifted == App(Var(3), Var(0))


def test_shift_through_lambda_increments_free_variable():
    term = Lam(TypeUniverse(), App(Var(1), Var(0)))
    shifted = shift(term, by=1, cutoff=0)
    assert shifted == Lam(TypeUniverse(), App(Var(2), Var(0)))


def test_subst_replaces_target_and_decrements_greater_indices():
    term = App(Var(1), Var(0))
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert result == App(Var(0), Succ(Var(0)))


def test_subst_under_lambda_preserves_bound_variable():
    term = Lam(TypeUniverse(), App(Var(1), Var(0)))
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert result == Lam(TypeUniverse(), App(Succ(Var(1)), Var(0)))


def test_shift_nested_binders():
    term = Lam(TypeUniverse(), Lam(TypeUniverse(), Var(2)))
    shifted = shift(term, by=1, cutoff=0)
    assert shifted == Lam(TypeUniverse(), Lam(TypeUniverse(), Var(3)))


def test_subst_nested_binder_chain():
    term = Lam(TypeUniverse(), Lam(TypeUniverse(), Var(2)))
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert result == Lam(TypeUniverse(), Lam(TypeUniverse(), Succ(Var(2))))


def test_subst_pi_body():
    pi_term = Pi(TypeUniverse(), Pi(TypeUniverse(), Var(1)))
    sub = Succ(Var(0))
    result = subst(pi_term, sub)
    assert result == Pi(TypeUniverse(), Pi(TypeUniverse(), Var(1)))


def test_subst_sigma_pair():
    sigma_term = Sigma(TypeUniverse(), Var(0))
    pair = Pair(Succ(Var(1)), Var(0))
    result = subst(pair, sigma_term)
    assert result == Pair(Succ(Var(0)), Sigma(TypeUniverse(), Var(0)))


def test_subst_natrec_components():
    term = NatRec(
        Pi(TypeUniverse(), TypeUniverse()), Zero(), Lam(TypeUniverse(), Var(0)), Var(0)
    )
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert isinstance(result, NatRec)


def test_subst_identity_constructs():
    term = Id(TypeUniverse(), Var(0), Var(1))
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert isinstance(result, Id)


def test_subst_idelim_components():
    term = IdElim(TypeUniverse(), Var(0), Var(1), Var(2), Var(3), Var(4))
    sub = Succ(Var(0))
    result = subst(term, sub)
    assert isinstance(result, IdElim)