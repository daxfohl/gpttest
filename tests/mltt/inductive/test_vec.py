...
from mltt.inductive.vec import ConsCtor, NilCtor, Vec, VecType


def test_infer_vec_type() -> None:
    assert Vec.infer_type() == mk_pis(Univ(0), NatType(), return_ty=Univ(0))
...
def test_scrut_type() -> None:
    ...
    t = scrut.infer_type()
    assert t == mk_app(Vec, NatType(), Zero())
    scrut = vec.Cons(NatType(), Zero(), Zero(), scrut)
    t = scrut.infer_type()
    assert t == mk_app(Vec, NatType(), Succ(Zero()))
