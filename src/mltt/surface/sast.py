...
class SurfaceTerm:
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        raise SurfaceError("Unsupported surface term", self.span)

    def elab_check(self, env: Env, expected: Term) -> Term:
        term, term_ty = self.elab_infer(env)
        if not term_ty.type_equal(expected, env):
            raise SurfaceError("Type mismatch", self.span)
        return term
...
class SBinder:
    ...
    def elab(self, env: Env) -> tuple[Term, Env]:
        if self.ty is None:
            raise SurfaceError("Missing binder type", self.span)
        ty_term, _ = self.ty.elab_infer(env)
...
class SAnn(SurfaceTerm):
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        ty_term, _ = self.ty.elab_infer(env)
        _ = ty_term.expect_universe(env)
        term = self.term.elab_check(env, ty_term)
        return term, ty_term
...
class SLam(SurfaceTerm):
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        ...
        binder_tys, env1 = _elab_binders(env, self.binders)
        body_term, body_ty = self.body.elab_infer(env1)
        lam_term = body_term
        lam_ty = body_ty
    ...
    def elab_check(self, env: Env, expected: Term) -> Term:
        ...
        for binder in self.binders:
            ...
            if binder.ty is None:
                ...
                binder_ty, _ = binder.ty.elab_infer(env1)
        ...
        body_term = self.body.elab_check(env1, expected_ty)
        lam_term = body_term
        for ty in reversed(binder_tys):
            lam_term = Lam(ty, lam_term)
        return lam_term
...
class SPi(SurfaceTerm):
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        binder_tys, env1 = _elab_binders(env, self.binders)
        body_term, _ = self.body.elab_infer(env1)
...
class SUApp(SurfaceTerm):
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        ...
        head_term, _ = self.head.elab_infer(env)
...
class SLet(SurfaceTerm):
    ...
    def elab_infer(self, env: Env) -> tuple[Term, Term]:
        ty_term, _ = self.ty.elab_infer(env)
        _ = ty_term.expect_universe(env)
        val_term = self.val.elab_check(env, ty_term)
