...
class ElabState:
    ...
    def add_constraint(self, env: Env, lhs: Term, rhs: Term, span: Span | None) -> None:
        ...
        from mltt.solver import solver as solver_module

        if solver_module.solve_meta(self, env, constraint, lhs, rhs):
            return
    ...
    def solve(self, env: Env) -> None:
        ...
        solver_module.solve(self, env)

    def zonk(self, term: Term) -> Term:
        from mltt.solver import solver as solver_module

        return solver_module.zonk(self, term)

    def zonk_level(self, level: LevelExpr) -> LevelExpr:
        from mltt.solver import solver as solver_module

        return solver_module.zonk_level(self, level)

    def ensure_solved(self) -> None:
        from mltt.solver import solver as solver_module

        solver_module.ensure_solved(self)
