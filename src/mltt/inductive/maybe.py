...
@cache
def _maybe_family(level: LevelExpr | int) -> tuple[Ind, Ctor, Ctor]:
...
def MaybeAt(level: int | LevelExpr = 0) -> Ind:
    return _maybe_family(level)[0]


def NothingCtorAt(level: int | LevelExpr = 0) -> Ctor:
    return _maybe_family(level)[1]


def JustCtorAt(level: int | LevelExpr = 0) -> Ctor:
    return _maybe_family(level)[2]


def MaybeType(elem_ty: Term, *, level: int | LevelExpr = 0) -> Term:
    return App(MaybeAt(level), elem_ty)


def Nothing(elem_ty: Term, *, level: int | LevelExpr = 0) -> Term:
    return App(NothingCtorAt(level), elem_ty)


def Just(elem_ty: Term, value: Term, *, level: int | LevelExpr = 0) -> Term:
