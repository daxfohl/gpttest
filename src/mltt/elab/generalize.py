...
class Generalizer:
    ...
    def generalize_let(self, ty: Term, value: Term) -> tuple[int, Term, Term]:
        ...
        for members in groups.values():
            infos = [self._solver.level_metas.get(mid) for mid in members]
        ...
        if mapping:
            self._solver.constraints = [
                ...
                for c in self._solver.constraints
            ]
            self._solver.level_constraints = [
                LevelConstraint(
                ...
                for c in self._solver.level_constraints
            ]
        for group in to_generalize:
            for mid in group:
                self._solver.level_metas.pop(mid, None)
    ...
    def merge_type_level_metas(self, terms: list[Term]) -> list[Term]:
        ...
        for mid in meta_ids:
            if mid in seen:
                continue
            info = self._solver.level_metas.get(mid)
    ...
    def _level_meta_equalities(self) -> set[tuple[int, int]]:
        ...
        for constraint in self._solver.constraints:
