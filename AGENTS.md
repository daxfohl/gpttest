...
- De Bruijn strategy: binders push indices outward (0 = innermost). When extending contexts, shift existing entries so outer references remain stable. Substitutions decrement indices above the target and shift the inserted term when descending under binders. Inductive parameters are outermost, followed by indices, then constructor arguments; utilities like `instantiate_into` expect substitutions in that order and rely on consistent shifting.
